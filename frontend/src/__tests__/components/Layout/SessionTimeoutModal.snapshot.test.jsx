...
// Mock dependencies to ensure consistent snapshots
vi.mock("react-modal", () => ({
  default: ({
    children,
    isOpen,
...
describe("SessionTimeoutModal Snapshots", () => {
  ...
  it("matches snapshot when modal is open - desktop view", () => {
  ...
  it("matches snapshot for tablet view", () => {
  ...
  it("matches snapshot with French language", () => {
    const { container } = render(
      <SessionTimeoutModal {...mockProps} currentLang="fr" />,
    );
    expect(container.firstChild).toMatchSnapshot();
  });
});
